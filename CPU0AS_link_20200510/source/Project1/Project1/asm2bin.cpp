...
int rpls(string* new_str, string t, flg_list *flg_head);
...
void add_link(asm_list *asm_head)
{
	...
	char buff;
	fstream file;
	file.open("./config/link_code.txt", ios::in);
...
void add_ISR_label(asm_list* asm_head, flg_list* flg_head) {
	asm_list* asm_cur = asm_head;
	flg_list* flg_cur = flg_head;
	int ISR_check[ISR_num];
	memset(ISR_check, 0, ISR_num);
	while (flg_cur != NULL) {
	...
	while (!end) {
		if (state == 0) {
			if (asm_cur->code == "ORG" || asm_cur->code == "org") {
				...
				string tt;
				rpls(&tt, asm_cur->next->code, flg_head);
				...
				if (state == 1) {
					if (type == 1) {
						...
						if (!flag) {
							string tt;
							int find = rpls(&tt, asm_cur->next->code, flg_head);
							if(find)
								bin_buff += ((calculator(0, tt.size() - 1, tt.c_str()) - pc - 4) & 0xFFFFFF);
							else
								bin_buff += (calculator(0, tt.size() - 1, tt.c_str()) & 0xFFFFFF);
						...
						if (!flag) {
							...
							if (!flag) {
								...
								if (!flag) {
									...
									string tt;
									rpls(&tt, asm_cur->next->next->next->code, flg_head);
